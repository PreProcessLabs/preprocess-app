...
class Agent : ObservableObject {
    ...
    func setup(key: String? = nil) async -> Bool {
        if key != nil {
            keychain.set(key!, forKey: "PREPROCESS_LLM_KEY")
        }
        var did_setup = false
        let apiKey = key != nil ? key : keychain.get("PREPROCESS_LLM_KEY")
        if apiKey != nil {
            if FileManager.default.fileExists(atPath: apiKey!) {
                ...
                else
                {
                    ...
                    keychain.delete("PREPROCESS_LLM_KEY")//not a valid cred
                }
            } else {
                ...
                do {
                    ...
                    self.keychain.delete("PREPROCESS_LLM_KEY")// not a valid cred
                }
    ...
    ///
    func query(request: String, over: [PreProcessInterval]) async {
