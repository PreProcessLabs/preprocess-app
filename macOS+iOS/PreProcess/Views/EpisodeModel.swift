...
class EpisodeModel: ObservableObject {
    ...
    @Published var intervals: [PreProcessInterval] = []
    ...
    @MainActor func performRefreshData() {
        ...
        if self.filter.count < 3 {
            ...
            let potentials: [PreProcessInterval] = Memory.shared.search(term: self.filter)
            withAnimation(.easeInOut(duration: 0.3)) {
                intervals = potentials.filter { (interval: PreProcessInterval) in
