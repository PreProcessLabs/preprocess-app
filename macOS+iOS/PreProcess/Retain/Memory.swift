...
class Memory {
    ...
    private let defaults = UserDefaults(suiteName: "group.io.preprocess.ios")!
    
    ///
    /// Close any in-progress episodes (in case PreProcess was not properly shut down)
    ...
    func reload() throws {
        let url: URL = homeDirectory().appendingPathComponent("PreProcessMemory.sqlite3")
    ...
    func migrate() {
        if intervalDb!.userVersion == 0 {
            ...
            var intervals: [PreProcessInterval] = []
            do {
                ...
                while let interval = try stmt.failableNext() {
                    ...
                    if ep != nil {
                        let inter = PreProcessInterval(from: Date(timeIntervalSinceReferenceDate:interval[0] as! Double), to: Date(timeIntervalSinceReferenceDate:interval[1] as! Double), episode: ep!, document: interval[3] as! String)
    ...
    ///
    func browserAwareContext(front: NSRunningApplication, window_title: String) -> PreProcessAppContext {
        ...
        return PreProcessAppContext(front: front, title: title, context: context, isPrivate: isPrivate)
    }
#endif
    ///
    /// Check the currently active app, if different since last check
    /// then close the current episode and start a new one
    /// Ignores the main bundle (PreProcess) - creates sometimes undiscernable
    ...
    func updateActiveContext(windowTitles: Dictionary<String, String>, bundleId: String = "") {
...
#else
        let ctx = PreProcessAppContext(front: iRunningApplication(bundleID: bundleId, isActive: true, localizedName: bundleId), title: bundleId, context: bundleId, isPrivate: false)
#endif
        ...
        if ctx.front.isActive && (currentContext != ctx.context || currentContextIsPrivate != ctx.isPrivate) {
            ...
            let is_main_bundle = (currentContext == Bundle.main.bundleIdentifier) || (currentContext == "io.preprocess.ios")
    ...
    func trackFileChanges(ep: Episode) {
        ...
        if defaults.bool(forKey: "PREPROCESS_TRACK_FILES") {
    ...
    private func runRetention() {
        ...
        let retention = defaults.integer(forKey: "PREPROCESS_RETENTION")
        if retention == 0 {
            // retain forever
...
#endif
        let newItem = PreProcessInterval(
            from: at,
    ...
    ///
    func search(term: String, expand_by: Int = 0) -> [PreProcessInterval] {
        var result: [PreProcessInterval] = []
        do {
            ...
            while let interval = try stmt.failableNext() {
                ...
                if ep != nil {
                    let inter = PreProcessInterval(from: Date(timeIntervalSinceReferenceDate:interval[0] as! Double), to: Date(timeIntervalSinceReferenceDate:interval[1] as! Double), episode: ep!, document: interval[3] as! String, snippet: interval[4] as? String)
                    result.append(inter)
                } else {
    ...
    func insert(interval: PreProcessInterval) {
        do {
